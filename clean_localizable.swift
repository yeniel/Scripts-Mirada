...
var keysThatCanBeDead: [String] = [
    "Channel List",
    "DAILY_KEY",
    "Protect recording",
    "IRIS_ERROR_LOGIN_WRONG_PASSWORD_ERROR_CODE",
    "EVERY_MON_FRI_KEY",
    "SECOND",
    "Record periodic",
    "EVERY_KEY",
    ...
    "Favourites_key",
    "Record",
    "DELETE_ENTIRE_SEASON_KEY",
    "RECORD_SERIES_FROM_CURRENT_SEASON_KEY",
    "AD_MULTI_WILL_END",
    "Record series",
    "DELETE_ENTIRE_SERIES_KEY",
    "EVERY_SAT_SUN_KEY",
    "VERSION_FOOTER_LABEL",
    "Tomorrow",
    "IRIS_ERROR_LOGIN_WRONG_PASSWORD",
    "Resolve conflict",
    "SECONDS",
    "RECORD_SERIES_FROM_LAST_SEASON_KEY",
    "AD_SINGLE_WILL_END"
]
...
func localizedStringsInCode() -> [LocalizationCodeFile] {
    return executableFiles.compactMap {
        ...
        let matches = regexFor("\"(?!.*?%d)(.*?)\"", content: content)
...
func validateMissingKeys(_ codeFiles: [LocalizationCodeFile], localizationFiles: [LocalizationStringsFile]) {
    ...
    var allLocalizableKeys: [String] = []
    
    localizationFiles.forEach {
        allLocalizableKeys += $0.keys
    }
    
    let baseKeys = Set(allLocalizableKeys)
...
func validateDeadKeys(_ codeFiles: [LocalizationCodeFile], localizationFiles: [LocalizationStringsFile]) {
    ...
    let baseKeys = Set(allLocalizableKeys)
    let keysThatCanBeDeadSet = Set(keysThatCanBeDead)
    let allCodeFileKeys = codeFiles.flatMap { $0.keys }
    let deadKeys = baseKeys.subtracting(allCodeFileKeys).subtracting(keysThatCanBeDeadSet)
    if !deadKeys.isEmpty{
        printPretty("error: \(deadKeys) - Suggest cleaning dead keys")
//        abort()
    }
}
