...
changesetNotMerged=$(hg merge -P aae8a9f7a238 | grep "changeset" | cut -f3 -d: | tr '\n' '+' | sed 's/.$//g')
...
echo "SUBJECT=Commits de $1 no mergeados en $currentBranch" >> email.txt

if [ -z "$commitsNotMerged" -o ! -n "$commitsNotMerged" ]; then
